...
import { ArrowLeft, Trophy, Sparkles, Heart, TrendingUp, Edit, ShoppingBag } from 'lucide-react';
...
// Importar badges da loja
const shopBadges = [
  // Latão (100 gemas)
  { id: 'brass-explorer', name: 'Explorador de Latão', tier: 'brass', price: 100, icon: '🗺️', color: 'from-[#b08d57] to-[#9a7b4f]', borderColor: 'border-[#7d6841]' },
  { id: 'brass-reader', name: 'Leitor de Latão', tier: 'brass', price: 100, icon: '📖', color: 'from-[#b08d57] to-[#9a7b4f]', borderColor: 'border-[#7d6841]' },
  { id: 'brass-math', name: 'Matemático de Latão', tier: 'brass', price: 100, icon: '🔢', color: 'from-[#b08d57] to-[#9a7b4f]', borderColor: 'border-[#7d6841]' },
  
  // Bronze (200 gemas)
  { id: 'bronze-warrior', name: 'Guerreiro de Bronze', tier: 'bronze', price: 200, icon: '⚔️', color: 'from-[#cd7f32] to-[#b87333]', borderColor: 'border-[#8b5a2b]' },
  { id: 'bronze-scientist', name: 'Cientista de Bronze', tier: 'bronze', price: 200, icon: '🔬', color: 'from-[#cd7f32] to-[#b87333]', borderColor: 'border-[#8b5a2b]' },
  { id: 'bronze-linguist', name: 'Linguista de Bronze', tier: 'bronze', price: 200, icon: '🌐', color: 'from-[#cd7f32] to-[#b87333]', borderColor: 'border-[#8b5a2b]' },
  
  // Prata (300 gemas)
  { id: 'silver-champion', name: 'Campeão de Prata', tier: 'silver', price: 300, icon: '🏆', color: 'from-[#c0c0c0] to-[#a8a8a8]', borderColor: 'border-[#808080]' },
  { id: 'silver-genius', name: 'Gênio de Prata', tier: 'silver', price: 300, icon: '🧠', color: 'from-[#c0c0c0] to-[#a8a8a8]', borderColor: 'border-[#808080]' },
  { id: 'silver-artist', name: 'Artista de Prata', tier: 'silver', price: 300, icon: '🎨', color: 'from-[#c0c0c0] to-[#a8a8a8]', borderColor: 'border-[#808080]' },
  
  // Ouro (400 gemas)
  { id: 'gold-master', name: 'Mestre de Ouro', tier: 'gold', price: 400, icon: '👑', color: 'from-[#ffd700] to-[#ffcc00]', borderColor: 'border-[#daa520]' },
  { id: 'gold-legend', name: 'Lenda de Ouro', tier: 'gold', price: 400, icon: '⭐', color: 'from-[#ffd700] to-[#ffcc00]', borderColor: 'border-[#daa520]' },
  { id: 'gold-prodigy', name: 'Prodígio de Ouro', tier: 'gold', price: 400, icon: '💫', color: 'from-[#ffd700] to-[#ffcc00]', borderColor: 'border-[#daa520]' },
  
  // Platina (500 gemas)
  { id: 'platinum-supreme', name: 'Supremo de Platina', tier: 'platinum', price: 500, icon: '💎', color: 'from-[#e5e4e2] to-[#d3d3d3]', borderColor: 'border-[#b0b0b0]' },
  { id: 'platinum-transcendent', name: 'Transcendente de Platina', tier: 'platinum', price: 500, icon: '🌟', color: 'from-[#e5e4e2] to-[#d3d3d3]', borderColor: 'border-[#b0b0b0]' },
  { id: 'platinum-immortal', name: 'Imortal de Platina', tier: 'platinum', price: 500, icon: '✨', color: 'from-[#e5e4e2] to-[#d3d3d3]', borderColor: 'border-[#b0b0b0]' },
];
...
export function ProfilePage({ user, onBack, onUpdateAvatar }: ProfilePageProps) {
  ...
  // Badges compradas na loja
  const purchasedBadges = shopBadges.filter(badge => user.badges?.includes(badge.id));
          ...
          <div className="bg-[#f5f1e3] border-4 border-[#3e2723] p-6 pixel-shadow-lg">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="bg-gradient-to-b from-[#ffcc33] to-[#ff9933] hover:brightness-110 px-4 py-2 border-4 border-[#d4a02c] pixel-button transition-all flex items-center gap-2"
              >
                <ShoppingBag className="w-4 h-4 text-white" />
                <span className="text-white drop-shadow-[1px_1px_0_rgba(0,0,0,0.5)]">Ir para Loja</span>
              </button>
            </div>

            {/* Badges Compradas - Estilo GitHub */}
            {purchasedBadges.length > 0 && (
              <div className="mb-6">
                <h3 className="mb-3 text-[#3e2723]">🏆 Insígnias da Loja</h3>
                <div className="bg-white border-4 border-[#3e2723] p-4">
                  <div className="flex flex-wrap gap-2">
                    {purchasedBadges.map((badge) => (
                      <div
                        key={badge.id}
                        className={`bg-gradient-to-b ${badge.color} ${badge.borderColor} border-4 p-3 pixel-shadow-sm hover:scale-110 transition-transform cursor-pointer group relative`}
                        title={badge.name}
                      >
                        <div className="text-3xl">{badge.icon}</div>
                        {/* Tooltip */}
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                          <div className="bg-[#3e2723] text-white px-3 py-2 border-2 border-white whitespace-nowrap text-sm">
                            {badge.name}
                          </div>
                        </div>
                      </div>
                  ...
                  <p className="text-[#6d4c41] text-sm mt-3">
                    {purchasedBadges.length} {purchasedBadges.length === 1 ? 'insígnia conquistada' : 'insígnias conquistadas'} na loja
                  </p>
                </div>
              </div>
            )}
